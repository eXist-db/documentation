...
declare function dq:main($node as node()*, $params as element(parameters)?, $model as item()*) {
    let $askPass :=
        not(xdb:collection-available($dq:COLLECTION))
    return
    	transform:transform(dq:get-page((), $askPass)/chapter, doc('/db/doc/stylesheets/db2xhtml.xsl'), ())
};